...
async def run(mcp_servers: list[MCPServer]):
    ...
    agent = Agent(
        name="Office Genius Agent",
        instructions="You are a helpful AI assistant. Use the available tools to interact with the filesystem (e.g., read files, list directories) and manage PowerPoint presentations (e.g., create presentations, add slides, save files) to assist the user with their requests. Respond to the user based on the information you gather or the actions you perform.",
        model=OpenAIChatCompletionsModel(model=os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT_NAME"), 
                                         openai_client=azure_open_ai_client),
        mcp_servers=mcp_servers,
    )

    print("AI 助理已準備就緒。輸入「離開」或「退出」來結束對話。")
...
async def main():
    ...
    async with filesystem_server, ppt_server:
        await run([filesystem_server, ppt_server])
